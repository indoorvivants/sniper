//> using scala 3.7.0-RC2
//> using platform scala-native
//> using nativeVersion 0.5.7

//> using dep com.github.lolgab::magnum::1.3.1
//> using dep com.github.lolgab::scala-native-jdbc-sqlite::0.0.3
//> using dep com.indoorvivants::decline-derive::0.3.1
//> using dep tech.neander::cue4s::0.0.7+3-0e957ac2-SNAPSHOT
//> using dep com.indoorvivants::toml::0.3.0
//> using dep com.outr::scribe::3.16.1
//> using dep com.lihaoyi::os-lib::0.11.4
//> using dep com.lihaoyi::pprint::0.9.0
//> using dep com.lihaoyi::upickle::4.1.0
//> using dep com.indoorvivants::trigs::0.0.2-SNAPSHOT
//> using dep net.andimiller::decline-completion::0.1.0
